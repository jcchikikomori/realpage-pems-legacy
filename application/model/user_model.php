...
class UserModel
{
    ...
    private $user_table = 'users'; // under alpha
    ...
    {
        //$sql = "SELECT * FROM users NATURAL JOIN tb_branch WHERE user_id != 1";
    ...
    {
        $sql = "SELECT COUNT(user_id) AS pending_users FROM users WHERE user_active = 0 AND user_activation_hash IS NOT NULL";
        $query = $this->db->prepare($sql);
        $query->execute();
    ...
    {
        $sql = "SELECT COUNT(user_id) AS user_count FROM users WHERE user_id != 1";
        $query = $this->db->prepare($sql);
        $query->execute();
    ...
    {
        $sql = "SELECT * FROM users NATURAL JOIN tb_branch WHERE user_id = :user_id LIMIT 1";
        $query = $this->db->prepare($sql);
    ...
    {
        $query = $this->db->prepare("SELECT user_id, user_creation_timestamp FROM users");
        $query->execute();
    ...
    public function login() {
        ...
        $query = $this->db->prepare("SELECT users.*
                FROM users ".
                // "WHERE users.user_provider_type != '' AND (user_name = :user_name OR user_email = :user_name)"); // disabling email login for now
                "WHERE users.user_provider_type != '' AND user_name = :user_name");
    ...
    public function registerNewUser($activate = false) {
        ...
        if (empty($_POST['user_provider_type'])) {
            ...
            // write new users data into database
            $sql = "INSERT INTO users (user_id, user_name, user_password, user_email, first_name, last_name, middle_name, branch_id, user_creation_timestamp, user_activation_hash, user_provider_type)
